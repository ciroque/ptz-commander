...
public:
	...
	enum DevicesState {
		...
		WifiStopWifiScan,        // stop wifi scan
	...
	public:
		...
		uint8_t pwd;          // wifi support 64 or not
	};
	...
	/**
         * @brief This callback function is triggered when a plug-in
         *        event of a device is failed. Register by setDevConnectFailedCallback function
         * @param [in] DevConnectFailed   	The Reason of devConnect Failed
		 *
		 * @param [in] string        		The SN of the current device is a 14-digit
         *                           		string used to uniquely identify the device.
         * @param [in] void *        		Custom parameters passed in by the user
         *
         */
	typedef std::function<void(DevConnectFailed, std::string,
				   Device::DevMode, void *)>
		devConnectFailedCallback;
