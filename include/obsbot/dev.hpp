...
#define FAST_UVC_DEV_CAM_STATUS_REFRESH_PERIOD (25)
...
#define CONFIG_REMO_MAX_BT_DEV_NUM 2
...
enum DevProgressType {
	...
	kErrCodeSwitchMtpFailed = -4,
	kErrCodeGetMtpDevTimeout = -5,
	kErrCodeGetUvcDevTimeout = -6,
	kErrCodeUploadFwFailed = -7,
	kErrCodeGetUpgradeResTimeout = -8,
	kErrCodeGetUpgradeResFailed = -9,
};
...
// DevConnect Failed Reason
enum class DevConnectFailed {
	GetUUidFailed = 0,  // get devUUid Failed
	GetSnFailed,        // get devSn Failed
	GetDevVersion,      // get devVersion Failed
	NetConnectFailed,   // net connect Failed
	CameraStatusfailed, //device wait camera status failed
};
...
struct transParentCommitCameraStatusCache {
	std::string zooming{"null"};
	std::string zoomSpeed{"null"};
	std::string zoomSettingCurrent{"null"};
	std::string deviceModel{"null"};
	std::string pan_motor_d{"null"};
	std::string pitch_motor_d{"null"};
	std::string main_mode{"null"};
	std::string sub_mode{"null"};
};
...
public:
	...
	/**
          * @brief  This callback function is used to periodically push the
          *         current device status to the outside. The push cycle is
          *         about every two or three seconds, fast status.
          * @param  param   User-defined parameter.
          * @param  data    The device status info, refer to CameraStatus.
          */
	typedef std::function<void(void *param, const void *data,
				   const std::string &dev_sn)>
		FastDevStatusCallback;

	/**
          * @brief  This callback function is used to obtain the range parameters
          *         related to the device system, peripherals, and media.
          * @param  param   User-defined parameter.
          * @param  data    The device status info, refer to DevConfigRangeInfo.
          */
	typedef std::function<void(void *param, const void *data)>
		DevConfigRangeCallback;
	...
	/**
          * @brief  This callback function is used to asynchronously process the
          *         received messages with special length from the device.
          * @param  param       User-defined parameter.
          * @param  rcvd_data   Received message from the device, message type
          *                     is related to the specific calling function.
          *                     The first data (u8) is the data length (>= 0) or
	...
	enum RmEventType {
		...
		kEvtWarnNoAudioInput,       /// no audio input
		kEvtWarnMtpStreamFailed,    /// mtp start stream failed
		kEvtWarnMediaStreamParamIneffective1, /// hdmi notice, blank screen
		kEvtWarnMediaStreamParamIneffective2, /// hdmi notice, not support
		kEvtWarnUsbcVolumeSetUnsuportted, // usbc mic set volume not support
		kEvtWarnHDMIResolutionNotSupport, // hdmi resolution not support

		kEvtInfoMicPlugin = 2000, /// mic in plugin
		kEvtInfoMicUnplug,        /// mic in unplug
		kEvtInfoLinePlugin,       /// line in plugin
		kEvtInfoLineUnplug,       /// line in unplug
		...
		kEvtInfoHDRWith60Fps, /// for tiny2, fps is 60 when HDR is enabled
		kEvtInfoSyncBootState, /// the presets are synchronized to the initial state
		kEvtInfoHDRWithPortrait, /// the screen mode is portrait when hdr is enabled
		kEvtInfoBluetoothMatchFailed, /// bluetooth match failed
		kEvtInfoHDRWith100Fps, /// for meetSE、tinySE, fps is larger than 100 when HDR is enabled
		kEvtInfoMtpStreamResume, /// mtp start stream resume
		kEvtInfoHdrEffectChange,
		kEvtInfoNightSceneTips,
		kEvtInfoNightSceneStatusChange,
		kEvtInfoNightSceneEffectChange,
		kEvtInfoPCRecord,        /// trigger pc record
		kEvtInfoUsbCAudioPlugin, /// USB-C audio plugged in
		kEvtInfoUsbCAudioUnplug, /// USB-C audio unplugged
		kEvtInfoSdRecordNoAudio, //  Record no Audio
		...
		kEvtTipsUvcStatus,        /// device uvc switch status
		kEvtTipsNdiStatus,        /// device ndi switch status
		kEvtTipsRtspStatus,       /// device rtsp switch status
		kEvtTipsSrtStatus,        /// device srt switch status
		kEvtTipsRemoteUpStatus,   /// remote controller upgrade status
		kEvtTipsAdapterPower,     /// adapter power status
		kEvtTipsGammaAnchorToMax, /// add gamma anchor failed
		kEvtTipsDevStatusChange,  /// presets settings dev status change
		kEvtTipsZoomOverRange,    /// zoom out of range
		kEvtTipsNdiActivateInfo,  /// note to activate NDI service
		kEvtTipsTWSFirstConnect,  /// TWS first connect
		kEvtTipsTWSTipConnect1,   /// TWS connect 1
		kEvtTipsTWSTipConnect2,   /// TWS connect 2
		kEvtTipsTWSTipElectric1,  /// TWS electric 1
		kEvtTipsTWSTipElectric2,  /// TWS electric 2
		kEvtTipsTWSTipMute1,      /// TWS mute 1
		kEvtTipsTWSTipMute2,      /// TWS mute 2
		kEvtTipsTWSTipMuteQuiet1, /// TWS mute quiet 1
		kEvtTipsTWSTipMuteQuiet2, /// TWS mute quiet 2
		kEvtTipsTWSTipDenoiseOn,  /// TWS denoise on
		kEvtTipsTWSTipDenoiseOff, /// TWS denoise off
		kEvtTipsTWSTipRecordOn,   /// TWS record on
		kEvtTipsTWSTipRecordOff,  /// TWS record off
		kEvtTipsTWSTipHumanTrackOn,  /// TWS human track on
		kEvtTipsTWSTipHumanTrackOff, /// TWS human track off
		kEvtTipsTWSTipSingleTrack,   /// TWS single track
		kEvtTipsTWSTipMultipleTrack, /// TWS multiple track
	};
	...
	enum AiWorkModeType {
		...
		AiWorkModeSpeech,     /// Mode speech for Tiny3
		AiWorkModeButt,
		AiWorkModePortraitTrack = 14, /// Portrait tracking for tiny3
		AiWorkModeCustomize = 15,     /// Mode only change sub mode
	};

	/// Audio mode
	/// For tiny3, tiny3Lite.
	enum AudioModeType {
		AudioModeOmni = 0, /// Omnidirectional radio
		AudioModeStereo,   /// Stereo
		AudioModeFront,    /// Forward pointing
		AudioModeBack,     /// backward pointing
		AudioModeDipol,    /// Forward and backward pointing
		AudioModeMusic,    /// Mode music
		AudioModeButt,
	};

	typedef struct {
		uint8_t source; /// 暂时用0
		uint8_t mode;   /// Refer to AudioModeType
	} AudioMode;
	...
	enum AiTrackSubModeType {
		...
		/// Animal track
		AiTrackSubModeTypeAnimalNormal = 10,
		AiTrackSubModeTypeAnimalCloseUp = 13,
		AiTrackSubModeTypeAnimalCustomAutoZoom,
		/// Common object track
		AiTrackSubModeTypeCommonNormal = 20,
		AiTrackSubModeTypeCommonCloseUp = 23,
...
#pragma pack(1)
	...
	struct PresetsAction {
		...
		// 11: human single tracking			[Tiny3]
		// 12: human group tracking			[Tiny3]
	...
	/// Camera virtual track info, for tail air, tail2
	...
	typedef union {
		...
		struct {
			...
			struct {
				uint8_t source : 3; /// audio source type, refer to DevAudioSourceType
				uint8_t mode : 5; /// audio mode, refer to AudioModeType
			} audio_mode;
			struct {
				uint8_t is_tws_mode : 1; /// 0: 2.4G MIC mode, 1: BT TWS mode
				uint8_t tx_state : 2; /// 00 disconnected, 01: TX0 online, 10, TX1 online, 11 both online
				uint8_t is_pairing : 1; /// 0 BT not connect, 1: BT connect
				uint8_t is_scanning : 1; /// 0: not scanning, 1: scanning
				uint8_t reserve : 3;
			} wireless_mic; /// for tiny3, tiny3Lite
			struct {
				uint8_t landscape_auto_frame : 4;
				uint8_t portrait_auto_frame : 4;
			} auto_frame;
			uint8_t event_count; /// count of camera event
			struct {
				uint8_t tiny_extend : 1;
				uint8_t track_extend : 1;
				uint8_t kws_reserver : 6;
			} kws_extend;
			uint8_t led_enable;
			struct {
				uint8_t doa_find_back : 1; /// sound source assisted tracking
				uint8_t doa_range : 2;  /// doa range
				uint8_t disable_bf : 1; /// audio mode limit
				uint8_t reserved : 5;
			} doa_set; /// for tiny3, tiny3Lite
			uint8_t rvd[13];
		...
		struct {
			...
			struct {
				...
				uint8_t sd_type : 1; /// 1: is ExFat type; 0: si not ExFat type
				uint8_t sleep_micro : 1; /// 0->close microphone, 1->open microphone，for tail2s
				uint8_t dev_status : 1; /// 0->sleep, 1->wake，for tail2s
				uint8_t bg_img_idx : 2; /// current background index in sleep，for tail2s
				uint8_t bg_img_mirror : 1; /// 0->off, 1->on，for tail2s
				uint8_t live_stream_mode : 1; /// 0->disabled, 1->enabled，for tail2s
			} extern_flag;
			/// sleep background type: image or video.
			/// bit0~bit3: 1->image, 0-none;
			/// bit4~bit7: 1->video, 0->none.
			...
			struct {
				...
				uint8_t ndi_boot_enable : 1; /// no used
				uint8_t remote_start_record : 1; /// Remote control recording, for Tail2s
				uint8_t remote_start_shoot : 1; /// Remote control shoot, for Tail2s
				uint8_t night_scene_mode : 1; /// 0->close night scene mode, 1->open night scene mode
				uint8_t support_night_scene : 1; /// 0->not support night scene mode, 1->support night scene mode
				uint8_t rvd : 2;
			} boot_media_setting;
			struct {
			...
			struct {
				...
				uint16_t line_attached : 1; /// 1->3.5mm line in connected
				...
				/// 0-> close hdmi output,1->open hdmi output when dev sleep
				uint16_t hdmi_sleep_bg : 1;
				uint16_t usbc_audio_attached : 1; /// 1->usb-c audio connected
			} online_status;
			union {
				struct {
...
public:
	...
	/**
         ...
         *         function DevStatusCallback will be called. The value of the
         *         counter can be dynamically changed to control the time point
         *         when the next callback function is triggered, fast status.
         * @category  all.
         * @param  [in] value   The counter value.
         */
	void fastNextRefreshDevStatus(
		int32_t value = FAST_UVC_DEV_CAM_STATUS_REFRESH_PERIOD);
	...
	/**
         * @brief  Register DevStatusCallback function, refer to
         *         DevStatusCallback, fast status.
         * @category  all.
         * @param  [in] callback   The callback function to register.
         * @param  [in] param      User-defined parameter.
         */
	void setFastDevStatusCallbackFunc(FastDevStatusCallback callback,
					  void *param);
...
public:
	...
	typedef struct {
		int16_t roll_euler_d; // 欧拉角，单位度
		int16_t pitch_euler_d;
		int16_t pan_euler_d;
		int16_t roll_motor_d; // 关节角，单位度
		int16_t pitch_motor_d;
		int16_t pan_motor_d;
		int16_t roll_v; // 旋转速度，单位： 度 / s
		int16_t pitch_v;
		int16_t pan_v;
		int16_t roll_ref;    // ROLL 参考角度
		uint8_t lock;        // 云台是否锁定
		uint8_t base_status; // 云台底座的状态
		uint8_t vertical;    // 是否为竖屏
		uint8_t calibrate;   // 校准状态
		uint8_t sys_status;  // 系统状态
		int8_t roll_bias_h; // ROLL 在横/竖屏模式下的偏置角度
		int8_t roll_bias_v;
		uint8_t factory_check_status; // 工厂测试状态
		uint8_t user_accel_cali_percent; // 用户校准模式的进度百分比
		uint8_t pan_range_mode;          // PAN 工作范围模式
	...
	/**
         ...
         *                     1.0~2.0.
         * @param  [in] zoom_speed   The input value is normalized, valid range:
         *                     1~10, for tail2、tail2s.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t cameraSetZoomAbsoluteR(float zoom, int zoom_speed = -1);
	...
	/**
         * @brief  [internal] Get the current WDR state.
         * @category  tail air.
	...
	typedef struct {
		uint8_t kws_word; // UVC_AUDIO_CTRL_CMD_TINY = 0 UVC_AUDIO_CTRL_CMD_TRACK = 2
		uint8_t enable; // 1 enable extend action,  0 disable
	} KwsAction;
	/**
	...
	/**
         ...
         * @category  tiny2 series、tail series。
	...
	/**
         * @brief  [internal] Get the delay time in time-lapse photograph.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the boot mode info of the camera.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the split size of video file when recording.
         * @category  tail air.
	...
	// video encoder format, for tail air, tail2
	...
	/**
         * @brief  [internal] Get the encoder format for main video.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the video bitrate level for main video.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the video bitrate level for NDI and RTSP.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the encoder format for NDI and RTSP.
         * @category  tail air.
         * @param  [in] format   Refer to DevVideoEncoderFormat.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	...
	/**
         * @brief  [internal] Get the switch to select rtsp or ndi.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the mirror and flip state of the video image.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the rotation state of the device.
         * @category  tail air.
         * @param  [in] rotation   Refer to DevRotationState.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
...
#pragma pack()
	...
	/**
         * @brief  [internal] Get the info of hdmi.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the attribute of watermark.
         * @category  tail air.
	...
	enum DevSrtEncryType {
		DevSrtEncryTypeNone = 0,
		DevSrtEncryTypeAES128,
		DevSrtEncryTypeAES192,
		DevSrtEncryTypeAES256,
	};

	typedef struct DevMediaParamSrt {
		uint32_t cfg_id;
		DevSrtMode mode;
		std::string stream_id;
		uint32_t delay;
		DevSrtEncryType encry_type;
		std::string password;
		std::string name;
		uint32_t src_port;
		uint32_t dst_prot;
	...
	/// Camera media parameter setting
	////////////////////////////////////////////////////////////////////////

	enum DevMediaStreamId {
		DevMediaStreamIdAuto = 0,
		DevMediaStreamIdRecord,
		DevMediaStreamIdCapture,
		DevMediaStreamIdLive,
		DevMediaStreamIdRtsp,
		DevMediaStreamIdNdi,
		DevMediaStreamIdSrt,
		DevMediaStreamIdUvc,
		DevMediaStreamIdKcp,
		DevMediaStreamIdHdmi,
		DevMediaStreamIdSdi,
		/// for Night Mode
		DevMediaStreamIdRecordSub = 16,
		DevMediaStreamIdLiveSub = 17,
		DevMediaStreamIdNdiSub = 18,
	};

	enum DevMediaParamOperation {
		DevMediaParamOperationAuto = 0,
		DevMediaParamOperationStart,
		DevMediaParamOperationStop,
		DevMediaParamOperationPause,
		DevMediaParamOperationResume,
	};

	/**
         * @brief  Set the operation parameters of the stream, such as start, stop, pause... .
         * @category  tail2.
         * @param  [in] stream_id  refer to DevMediaStreamId.
         * @param  [in] action  Refer to DevMediaParamOperation.
         *
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t cameraSetMediaOperateParamR(DevMediaStreamId stream_id,
					    DevMediaParamOperation action);

	/**
         * @brief  Get the operation parameters of the stream, such as start, stop, pause... .
         * @category  tail2.
         * @param  [in] stream_id  refer to DevMediaStreamId.
         * @param  [out] action  Refer to DevMediaParamOperation.
         *
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t cameraGetMediaOperateParamR(DevMediaStreamId stream_id,
					    DevMediaParamOperation &action);

	enum DevMediaParamSdiMode {
		DevMediaParamSdiModeAuto = 0,
		DevMediaParamSdiModeLevel1,
		DevMediaParamSdiModeLevel2,
	};

	struct DevMediaParamSdi {
		int width;
		int height;
		int fps;
		bool iScan; // Interlaced scanning
		DevMediaParamSdiMode mode;
	};

	struct DevMediaParamHdmi {
		int width;
		int height;
		bool iScan; // Interlaced scanning
	};
	...
	/// Camera video config info
	typedef struct {
		DevMediaResolutionFps res_fps;
		DevVideoEncoderFormat enc_fmt;
		DevMediaRange bitrate_range;
	} DevVideoConfig;

	/// Camera media config info
	typedef struct {
		DevMediaStreamId stream_id;
		std::vector<DevMediaResolution> res_list;
		std::vector<int32_t> fps_list;
		std::vector<DevMediaResolutionFps> res_fps_list;
		std::vector<DevVideoEncoderFormat> enc_fmt_list;
		DevMediaRange bitrate_range;
		std::vector<DevVideoConfig> video_config_list;
	} DevMediaConfig;

	/// Camera config range info
	typedef struct {
		/// dev sn
		std::string dev_sn;
		/// Media info
		std::vector<DevMediaConfig> media_config;
		/// Support system information and device information when needed later
	} DevConfigRangeInfo;

	/**
         * @brief Get the range parameters of the device configuration.
         * @category  tail2.
         * @param  [in] callback   The callback function to register.
         * @param  [in] param      User-defined parameter.
         * @param  [in] data       In network mode, the device configuration information
         *                         obtained through http.
         * @param  [in] data_size  In network mode, the device configuration information size.
         *
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t cameraGetConfigRange(const DevConfigRangeCallback &callback,
				     void *param, const std::string &data = "",
				     int data_size = 0);

	typedef struct DevMediaEncodeParam {
		int32_t width = -1;
		int32_t height = -1;
		int32_t fps = -1;
		int32_t bitrate = -1;
		DevVideoEncoderFormat encode_format = DevVideoEncoderAuto;
	} DevMediaEncodeParam;

	/**
         * @brief  Set recording encoding parameters.
         * @category  tail2.
         * @param  [int] night_flag true->night mode, false->other mode
         * @param  [in] encode_param  Refer to DevMediaEncodeParam.
         *
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t
	cameraSetRecordEncodeParamR(const DevMediaEncodeParam &encode_param,
				    bool night_flag);

	/**
         * @brief  [internal] Get recording encoding parameters.
         * @category  tail2.
         * @param  [int] night_flag true->night mode, false->other mode
         * @param  [out] encode_param  Refer to DevMediaEncodeParam.
         *
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t cameraGetRecordEncodeParamR(DevMediaEncodeParam &encode_param,
					    bool night_flag);

	/**
         * @brief  Set output encoding parameters, such as rtsp, ndi, srt, etc.
         * @category  tail2.
         * @param  [int] night_flag true->night mode, false->other mode
         * @param  [in] encode_param  Refer to DevMediaEncodeParam.
         *
         ...
         */
	int32_t
	cameraSetOutputEncodeParamR(const DevMediaEncodeParam &encode_param,
				    bool night_flag);

	/**
         * @brief  [internal] Get output encoding parameters, such as rtsp, ndi, srt, etc.
         * @category  tail2.
         * @param  [int] night_flag true->night mode, false->other mode
         * @param  [out] encode_param  Refer to DevMediaEncodeParam.
         *
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t cameraGetOutputEncodeParamR(DevMediaEncodeParam &encode_param,
					    bool night_flag);

	/**
         * @brief  [internal] Get live stream encoding parameters.
         * @category  tail2.
         * @param  [int] night_flag true->night mode, false->other mode
         * @param  [out] encode_param  Refer to DevMediaEncodeParam.
         *
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t cameraGetLiveEncodeParamR(DevMediaEncodeParam &encode_param,
					  bool night_flag);
	...
	enum DevWhiteBalanceType {
		...
		DevWhiteBalanceRBGain = 253,
		DevWhiteBalanceArea = 254,
	...
	/**
         * @brief  [internal] Get the white balance param.
         * @category  tiny, tiny4k, tiny2, tail air, meet, meet4k.
	...
	typedef struct WhiteBalanceSetting {
		DevWhiteBalanceType light_mode;
		int32_t param;
		bool is_manual_gain;
		int32_t user_manual_b_gain;
		int32_t user_manual_r_gain;
		int32_t xab_offset;
		int32_t ygm_offset;
	} WhiteBalanceSetting;

	/**
	 * @brief  Get the white balance iq param.
	 * @category  Tail2.
	 * @param  [in] white_balance  Refer to DevWhiteBalance.
	 * @return  RM_RET_OK for success, RM_RET_ERR for failed.
	 */
	int32_t cameraGetWhiteBalanceR(WhiteBalanceSetting &balance_settings);

	/**
	 * @brief  Set the white balance iq param.
	 * @category  Tail2.
	 * @param  [in] white_balance  Refer to DevWhiteBalance.
	 * @return  RM_RET_OK for success, RM_RET_ERR for failed.
	 */
	int32_t
	cameraSetWhiteBalanceR(const WhiteBalanceSetting &balance_settings);
	...
	/**
         * @brief  [internal] Get the maximum and minimum ISO.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the AE lock state.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the enable state of face AE.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the shutter mode.
         * @category  tail air.
...
#define GAMMA_LUT_MAX_NUM 22
#pragma pack(1)
	typedef struct {
		uint8_t mode;                    /// 0 default, 1: user
		uint8_t count;                   /// anchor counts
		uint8_t lutX[GAMMA_LUT_MAX_NUM]; /// anchor pos x
		uint8_t lutY[GAMMA_LUT_MAX_NUM]; /// anchor pos y
	} DevGammaModeUserMode;
#pragma pack()
	...
	/**
         * @brief  [internal] Get the EVBIAS of auto exposure in P Gear. Backlight
         *         compensation Control in Uvc.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the shutter time of auto exposure in S Gear.
         * @category  tail air.
	...
	/**
         * @brief  [internal] Get the EVBIAS of auto exposure in A Gear.
         * @category  tail air.
         * @param  [out] ev_bias   Refer to DevAEEvBiasType.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	...
	/**
         * @brief  [internal] Get the shutter time of auto exposure in M Gear.
         * @category  tail air.
         * @param  [out] shutter_time   Refer to DevShutterTimeType.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	...
	/**
         * @brief  [internal] Get the ISO of auto exposure in M Gear.
         * @category  tail air.
         * @param  [out] iso   ISO.
         * @param  [in] user   user value or realtime value, for Net or Mtp Mode.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t cameraGetMAEIsoR(int32_t &iso, bool user = true);
	...
	typedef struct WhiteBalanceOffset {
		int32_t xab; // 0(-7) - 56(7), 0.25 per step
		int32_t ygm; // 0(-7) - 56(7), 0.25 per step
	} WhiteBalanceOffset;
	...
	/**
         * @brief  [internal] Get the focus motor position of the camera.
         * @category  tail air.
	...
	enum DevAFCType {
		...
		DevAFCForeground, /// foreground focus
	};
	...
	/**
         * @brief  [internal] Get the continue auto focus type of camera.
         * @category  tail, air, tail2.
         * @param  [out] face_focus   Refer to DevAFCType.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	...
	enum DevNightMode { DevNightModeStandard = 0, DevNightModeSuper };

	enum DevImageStyleMode : uint32_t {
		DevImageStyleModeStandard = 0,
		DevImageStyleModeStreaming,
		DevImageStyleModeCostume,
		DevImageStyleModeMakeup,
		DevImageStyleModeJewelry,
		DevImageStyleModeDefault = 0xFFFF,
	};
	enum DevImageRestoreMode {
		DevImageRestoreModeFocus = 0,
		DevImageRestoreModeExposure,
		DevImageRestoreModeWb,
		DevImageRestoreModeImage,
		DevImageRestoreModeButt = 255,
	};

	typedef struct DevLiveStreamModeSetting {
		DevImageStyleMode image_style;
		DevImageRestoreMode image_restore;
	} DevLiveStreamModeSetting;

	typedef struct DevAEIsoThreshold {
		uint32_t auto_min;
		uint32_t auto_max;
		uint32_t manual_min;
		uint32_t manual_max;
	...
	/**
         ...
         * @category  tiny, tiny4k, tiny2, meet, meet4k, tail air, tail2.
	...
	enum DevAudioSourceType {
		DevAudioSourceTypeBuildIn = 0,
		DevAudioSourceTypeAuxLine,
		DevAudioSourceTypeAuxMic,
		DevAudioSourceTypeBluetooth,
		DevAudioSourceTypeUsbC,
	};
...
#pragma pack()
	...
	typedef struct {
		uint32_t enabled;
		uint32_t only_once; /// true->take effect only once
		uint32_t second;
		uint32_t minute;
		uint32_t hour;
		/// repeat on which day, sunday is the first day
		uint32_t week_repeat[7];
	} DevScheduledInfo;
	...
	struct DevVirtualTrackGesture {
		bool virtual_track_gesture_1;
		bool virtual_track_gesture_2;
		bool virtual_track_gesture_3;
	};

	enum DevCDCNotifyType : uint32_t {
		DevCDCNotifyTypeAiTarget = 1 << 0, /// Ai target info notify.
		DevCDCNotifyTypeFreeD = 1 << 1,    /// FreeD info notify
		DevCDCNotifyTypeTWSAudio = 1 << 2, /// TWS audio info notify
	};

	enum RemoteCustomKey {
		RemoteCustomKeyNone = 0,
		RemoteCustomKeyControlLed,
		RemoteCustomKeyControlTallyled,
		RemoteCustomKeyControlBuzzer,
		RemoteCustomKeyControlZoom1x,
		RemoteCustomKeyControlGimbalPos,
		RemoteCustomKeyControlMax,
	};

	enum RemoteCustomDoubleKey {
		RemoteCustomDoubleKeyNone = 0,
		RemoteCustomDoubleKeyControlChangeTrack,
		RemoteCustomDoubleKeyControlRotateScreen,
		RemoteCustomDoubleKeyControlPop,
		RemoteCustomDoubleKeyControlMax,
	};

	typedef struct RemoteCustomKeyFun {
		RemoteCustomKey remote_custom_key;
		RemoteCustomDoubleKey remote_custom_double_key;
	} RemoteCustomKeyFun;

	enum RemoteCustomKey2 {
		RemoteCustomKey2None = 0,
		RemoteCustomKey2Living,
		RemoteCustomKey2DisableAudioCamera,
		RemoteCustomKey2DisableAudioApp,
		RemoteCustomKey2HdmiOutputCamera,
		RemoteCustomKey2HdmiOutputMinitor,
		RemoteCustomKey2OutputFullFrame,
		RemoteCustomKey2ControlLed,
	};

	typedef struct RemoteCustomKeyFun2 {
		RemoteCustomKey2 remote_custom_key;
	} RemoteCustomKeyFun2;
	...
	typedef struct {
		uint8_t support_auto;
		uint8_t is_auto;         // 0: not auto; 1: auto;
		uint8_t has_pair_record; // 0: not support auto; 1: support auto
	} AudioSelectAttr;
	...
	enum DevTWSKeyType : uint8_t {
		DevTWSKeyTrack = 0,   /// trigger track when click tws key
		DevTWSKeyTrackSwitch, /// trigger switch track	when click tws key
		DevTWSKeyZoomX1, /// trigger zoom to 1.0x when click tws key
		DevTWSKeyRecord, /// trigger pc record when click tws key
	};

	enum DevTWSFuncType : uint8_t {
		DevTWSFuncBtn = 0,
		DevTWSFuncShock,
		DevTWSFuncLed,
		DevTWSFuncNS,
		DevTWSFuncAutoSuspend,
		DevTWSFuncShutdown,
	};

	enum DevTWSSoundMode : uint8_t {
		DevTWSSoundMono = 0,
		DevTWSSoundStereo,
	};

	enum DevTWSNSLevel : uint16_t {
		DevTWSNSLevelLow = 1,
		DevTWSNSLevelMid,
		DevTWSNSLevelHigh,
	};
#pragma pack(1)
	typedef struct DevTWSInfo {
		int8_t key_cmd; /// refer to DevTWSKeyType
		struct {
			uint8_t button_enable : 1;
			uint8_t shock_enable : 1;
			uint8_t led_enable : 1;
			uint8_t sound_mode : 2; /// refer to DevTWSSoundMode
			uint8_t shout_down : 1;
			uint8_t reserver : 2;
		} func;
		...
		int8_t mic1_ns_level;
		int8_t mic2_ns_level;
		int8_t mic1_gain;
		int8_t mic2_gain;
		uint8_t mic1_batt_level;
		uint8_t mic2_batt_level;
		uint8_t mic1_chg_status;
		uint8_t mic2_chg_status;
		int8_t reserver[5];
	} DevTWSInfo;
#pragma pack()

	enum DevUvcMode : uint32_t {
		DevUvcModeBulk = 0,
		DevUvcModeIso,
	};

	enum ZoomParamType {
		PROTOCOL_SET_MANUAL_ZOOM_SPD,
		PROTOCOL_SET_ENABLE_DIGITAL_ZOOM,
	};

	typedef struct CameraZoomParam {
		uint8_t type; // ZoomParamType 枚举
		union {
			uint8_t manual_spd; // 参数范围 1 - 10
			uint8_t enable_digital; // 0: 关闭数字变焦 1: 开启数字变焦
		} param;
	} CameraZoomParam;

	typedef struct CameraZoomParamR {
		uint8_t manual_spd;
		uint8_t enable_digital;
		uint8_t reserve[8];
	} CameraZoomParamR;
	...
	enum AccessoryUgState {
		AccessoryUgNormal = 0,
		AccessoryUgDevUnKnown,
		AccessoryUgDevBusy,
		AccessoryUgBatteryLow,
		AccessoryUgDelay,
	};

	typedef struct {
		AccessoryType type;
		AccessoryUgState state;
	} AccessoryUgInfo;
	...
	/**
         ...
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t sysMgSetIndicatorStateR(uint8_t state_id);
	...
	/**
         ...
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t sysMgClearIndicatorStateR(uint8_t state_id);
...
#pragma pack(1)
	...
	typedef struct {
		uint8_t state; /// The state of ethernet, refer to DevEthernetState
		int8_t if_name[16];
		uint8_t ip_proto;
		uint8_t ipv4[4];
		uint8_t netmask[4];
...
#pragma pack()
	...
	typedef struct {
		uint8_t key;   // 默认为1
		uint8_t event; // 1:进入设置页面，0:退出设置页面
	} NoTrackData;
    ...
    */
	...
	enum DevTargetClassType {
		...
		DevTargetClassTypeHumanOrAnimal = 100,
		/// No target limit
		DevTargetClassTypeAll = 101,
	};
	...
	enum DevTargetZoomType {
		...
		/// Automatically adapt the camera zoom type
		DevTargetZoomTypeAdaptive = 99,
	};

	enum DevCustomizedZoomType {
		DevCustomizedZoomTypeDisable = 0,
		DevCustomizedZoomTypeCloseUp,
		DevCustomizedZoomTypeHalf,
		DevCustomizedZoomTypeP7,
		DevCustomizedZoomTypeP9,
		DevCustomizedZoomTypeFull,
		DevCustomizedZoomTypeP16,
		DevCustomizedZoomTypeP24
	};
	...
	/**
         ...
         * @param  [in] customized_zoom_type   Refer to DevCustomizedZoomType
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	int32_t
	aiSetTargetZoomTypeR(DevTargetZoomType zoom_type,
			     DevCustomizedZoomType customized_zoom_type =
				     DevCustomizedZoomTypeDisable);
	...
	enum DevGimbalParaType {
		...
		/// preset speed, float, [0.1, 0.2, 0.4, 0.6, 0.8]
	...
	/**
         * @brief  Set gimbal function parameters.
         * @category  Tail2 and later products fully support,
         *            when type is DevGimbalParaTypePresetSpeed, Tail air supports
         * @param  [in] type           Refer to DevGimbalParaType.
         * @param  [in] parameter      Determine the data type and value through type.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	...
	enum DevGimCtrlSpeedMode {
		DevGimCtrlSpeedModeSuperLazy = 0,
		DevGimCtrlSpeedModeLazy,
		DevGimCtrlSpeedModeSlow,
		DevGimCtrlSpeedModeFast,
		DevGimCtrlSpeedModeCrazy,
		DevGimCtrlSpeedModeCustom = 100
	};
	...
	enum DevControlParaType {
		...
		/// int, auto zoom mode
		DevControlParaTypeAutoZoomMode,
	...
	/**
         ...
         * @param  [in] para_type          Refer to DevControlParaType.
         * @param  [out] parameter         Determine the data type and value through type.
         * @param  [in] fault              When para_type >= DevControlParaTypeLimitPanMin and para_type <= DevControlParaTypeLimitPitchMax,
         *                                 get default value.
         * @return  RM_RET_OK for success, RM_RET_ERR for failed.
         */
	...
	int32_t aiGetControlParaR(DevControlTargetType target_type,
				  DevControlParaType para_type,
				  float &parameter, bool fault = false);
	int32_t aiGetControlParaR(DevControlTargetType target_type,
				  DevControlParaType para_type, int &parameter);
